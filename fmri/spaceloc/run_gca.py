            ...
            for drr in d_rois:
                ...
                for rcn, rc in enumerate(run_combos): #determine which runs to use for creating ROIs
                    for cc in cond:
                        ...
                        #Extract TS from rois
                        dorsal_phys = extract_roi_sphere(img4d,dorsal_coords[['x','y','z']].values.tolist()[0])
                        dorsal_phys = extract_cond_ts(dorsal_phys, psy)
                        print('dorsal TS extracted')

                        for vrr in v_rois:
                            ventral_coords = roi_coords[(roi_coords['index'] == rcn) & (roi_coords['task'] =='toolloc') & (roi_coords['roi'] ==vrr)]
                            ventral_phys = extract_roi_sphere(img4d,ventral_coords[['x','y','z']].values.tolist()[0])
                            ventral_phys = extract_cond_ts(ventral_phys, psy)

                            print('ventral TS extracted')
                            ...
                            gc_res_dorsal = grangercausalitytests(neural_ts[['ventral','dorsal']], 1)
                            
                            #calculate ventral GCA F-test
                            gc_res_ventral = grangercausalitytests(neural_ts[['dorsal','ventral']], 1)
